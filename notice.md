...
**spike 端（2026-08-06，`deepseek-v4-flash`，thinking 关闭）**
...
**App 端（2026-08-08，PRD §11 验收输入，三次运行）**

- 6 条全部命中，抽取完整性 6/6 × 3
- 归类、日期、`timeVague` 标黄均 3/3 稳定
- 唯一不稳定项：提醒只给日期不给时间时（「明天要交周报」），时间点在 18:00 / 23:59 之间摆动

**spike 通过不等于 App 通过。** `过 Q3 方案` 在 spike 里 9/9 归工作，同期 App 里 0/3 归创意 —— 因为两边的日历定义不是同一份。排查归类问题时先读设备上的真实配置（命令见上）。
